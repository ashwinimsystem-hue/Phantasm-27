...
/* Authoritative event pricing.
   Gender is participant information only and is NEVER a pricing input.
   Team pricing is determined only by the event's participation mode. */
...
function parseDetails(eventsDetail) {
  ...
  return eventsDetail.map((item) => typeof item === 'string'
    ? { title: item, raw: item }
    : { title: item?.title || item?.eventName || item?.eventId || '', raw: item }
  ).filter((item) => String(item.title || '').trim());
}
function detailFor(title, details) {
  const target = norm(title);
  return details.find((detail) => { const value = norm(detail.title); return value === target || value.includes(target) || target.includes(value); })?.raw || null;
}
...
function inferTeam(detail, fallbackTeam) { const size = explicitTeamSize(detail); return size != null ? size > 1 : Boolean(fallbackTeam); }
function inferParticipantCount(detail) { const size = explicitTeamSize(detail); return size != null ? Math.max(1, size) : 1; }
function calculateCanonicalAmount({ eventList = [], eventsDetail = [], teamName = '', teamId = '', fallbackAmount = 0 } = {}) {
  ...
  let amount = 0; const unknownEvents = [];
  for (const title of titles) {
    const rule = findRule(title);
    if (!rule) { unknownEvents.push(title); continue; }
    const detail = detailFor(title, details);
    if (rule.perPerson) amount += rule.solo * inferParticipantCount(detail);
    else { const team = inferTeam(detail, Boolean(teamName || teamId)); amount += team && rule.team != null ? rule.team : rule.solo; }
  }
  const fullyKnown = titles.length > 0 && unknownEvents.length === 0;
  return { amount: fullyKnown ? amount : Number.isFinite(Number(fallbackAmount)) ? Number(fallbackAmount) : 0, technicalAmount: amount, fullyKnown, unknownEvents };
}
